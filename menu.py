...
def iniciar():
    while True:
        ...
        if opcion == "1":
            ...
            ine      = helpers.leer_texto(3, 3 , "INE(2 int y 1 char)").upper()
            nombre   = helpers.leer_texto(2, 30, "Nombre de la persona").capitalize()
            apellido = helpers.leer_texto(2, 30, "Apellido de la persona").capitalize()
            ...
            cliente  = db.Clientes.buscar(ine)
            if cliente:
                nombre   = helpers.leer_texto(2, 30, f"Nombre de la persona [{cliente.nombre}]").capitalize()
                apellido = helpers.leer_texto(2, 30, f"Apellido de la persona [{cliente.apellido}]").capitalize()
